<script lang="ts">
	...
	import type { Entity, Participant, RollMode } from '../app';

	import { apiAddParticipantToCombat } from '../lib/api';
...
<div class="row">
	...
	<div class="icons">
		<slot />
		<!-- <button on:click={() => _handleClick('one')}><Rat /></button>
		<button on:click={() => _handleClick('max')}><Crown /></button>
		<button on:click={() => _handleClick('default')}><PlusSquare /></button> -->
	</div>
</div>

<script lang="ts">
	...
	let showModal = false;
	let combat_id = parseInt($page.url.searchParams.get('combat_id') || '1');
	...
	$: {
		if (browser && $combat && $combat.active_participant_id) {
			currentActiveParticipant = $combat.participants.find(
				(p) => p.participant_id == $combat.active_participant_id
			);
		}
	}
	...
	const advanceCombat = (e: Event) => {
		...
		set_active_participant($combat.combat_id, next_participant);
	};
</script>

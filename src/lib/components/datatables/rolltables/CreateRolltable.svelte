<script lang="ts">
	...
	// let rollTable: typeof props.rollTable;

	// $: {
	// 	rollTable = {
	// 		...props.rollTable,
	// 		rows: props.rollTable?.rows?.forEach((e) => e) || []
	// 	};
	// }
	...
	const handleSubmit = (e: Event) => {
		dispatch('createRolltable', { ...rollTable });
		rollTable = { name: '', rows: [] };
		open = false;
	};
	...
	const processOptions = (name: string, options: string): RollTableCreate => {
		return {
			name,
			rows: options.split('\n').map((opt) => {
...
<Dialog.Root bind:open>
	<Dialog.Trigger asChild let:builder>
		<Button builders={[builder]}><PlusCircle class="h-4 w-4 mr-1" />Add New</Button>
	</Dialog.Trigger>

<script lang="ts">
	...
	import type { RollTableUpdate, RollTableCreate } from '../../../../app';
	...
	type HasWeight = { weight?: number | null } & { [id: string]: any };
	type HasRows = { rows: HasWeight[] } & { [id: string]: any };

	let open: boolean = false;

	export let rolltable: RollTableUpdate | RollTableCreate;
	...
	const calcTotalWeight = (r: any) => {
		return r.rows.reduce(accumulate, 0);
	};
	const accumulate = (partialSum: number, row: HasWeight) => {
		return partialSum + (row.weight || 1);
	};

	const createOrUpdateRolltable = (r: RollTableCreate) => {
		if (/**Create*/ true) {
			return client.POST('/rolltable/', {
				body: {
					name: r.name,
					rows: r.rows ? [...r.rows] : []
				}
								...
								<Table.Cell>
									<DropdownMenu.Root>
										...
										<DropdownMenu.Content class="w-[160px]" align="end">
											...
											<DropdownMenu.Item
												on:click={() => {
													rolltable.rows = [...rolltable.rows, { ...row, rolltable_row_id: -1 }];
												}}
											>
												Make a copy
											</DropdownMenu.Item>

<script lang="ts">
	...
	const dataStore = writable<Message[]>([]);
	...
	const getMessages = async (page: number = 0, perPage: number = 20, q: string) => {
	...
	const modifyMessage = (message_id: number, message: string) => {
		return client
	...
	const columns = table.createColumns([
		table.column({
			accessor: ({ message_id }) => message_id,
	...
	$: {
		if (browser) {
			...
			$sortKeys;
			getMessages($pageIndex, $pageSize, $filterValue);
		}
	}
